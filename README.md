...
### VMS 대시보드
Django 기반의 선박 관리 시스템 대시보드입니다. 그라디언트 배경과 모던한 UI로 선박 정보와 배포 이력을 한눈에 확인할 수 있습니다.

![VMS Dashboard](docs/screenshots/vms-dashboard.png)

**주요 기능**:
- 📊 실시간 통계 (전체 선박 수, 온라인 선박 수, 최근 배포 수)
- 🚢 선박 목록 테이블 (이름, IMO 번호, 유형, VMS 버전, 온라인 상태)
- 📜 배포 이력 (버전, 상태, 배포 시각, 소요 시간)
- 🎨 그라디언트 UI (보라색 → 파란색)

### Shore Packaging ( 육상 패키지 빌드)
Python 스크립트(`build_package.py`)를 사용하여 인터넷 연결이 가능한 육상에서 오프라인 설치 패키지를 생성합니다.

![Shore Packaging](docs/screenshots/build-package.png)

**수행 작업**:
1. 📥 `pip download`로 의존성 패키지(Whl) 다운로드
2. 📦 Django 소스 코드 및 정적 파일 압축
3. 🔒 SHA-256 체크섬 생성 (무결성 보장)

### Ship Deployment (선박 배포 프로세스)
PowerShell 스크립트를 통한 자동화된 배포 프로세스입니다.

![Deployment Process](docs/screenshots/deployment-process.png)

**실행 단계**:
1. ✅ SHA-256 무결성 검증
2. 📸 현재 버전 스냅샷 생성
3. 📦 패키지 압축 해제 및 의존성 설치
4. 🔄 Junction Point 원자적 전환
5. 🏥 헬스 체크 및 자동 롤백

### API Verification (API 응답)
배포 후 자동화된 헬스 체크 및 상태 모니터링을 위한 API 응답입니다.

| Health Check API | Vessels List API |
|------------------|------------------|
| ![Health Check](docs/screenshots/health-check-api.png) | ![Vessels API](docs/screenshots/vessels-api.png) |
...
### 기술적 차별점 (Technical Differentiators)
...
본 프로젝트는 실제 엔터프라이즈 운영 환경에서 요구되는 안정성과 확장성을 고려하여 설계되었습니다.
...
---
**Author**: Kang (Software/DevSecOps Engineer) 
